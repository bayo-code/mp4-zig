...
const Self = @This();
const FnDecl = fn (ctx: *anyopaque, buf: []u8) anyerror!usize;

ctx: *anyopaque,
readFn: *const FnDecl,

fn MakeReaderDecl(comptime T: type) type {
    return fn (ctx: *T, buf: []u8) anyerror!usize;
}

/// Make sure `ctx` is a pointer and that implements the `Reader` interface
pub fn init(comptime T: type, ctx: *T) Self {
    if (!@hasDecl(T, "read")) {
        @compileError("This struct does not implement the Reader interface");
    }

    const Decl = MakeReaderDecl(T);

    if (@TypeOf(T.read) != Decl) {
        @compileError("The read implementation does not match!!!\n. Expected: " ++ @typeName(Decl) ++ "\nGot: " ++ @typeName(@TypeOf(T.read)));
    }

    return Self{
        .ctx = @ptrCast(*anyopaque, @alignCast(@alignOf(*anyopaque), ctx)),
        .readFn = @ptrCast(*const FnDecl, &T.read),
    };
}
...
test "reader" {
    const DummyIO = struct {
        ...
        pub fn reader(self: *@This()) Self {
            return Self.init(@This(), self);
        }
    };
    ...
    try std.testing.expect(try reader.read(buf[0..]) == 300);
}
